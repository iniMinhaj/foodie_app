...
- **JSON-file-backed mock persistence**, not in-memory-only: `JsonStorageService` reads/writes copies of `assets/mock/*.json` seeded into the app documents directory by `AssetSeeder`, so writes (register, place order, edit profile) actually persist across app restarts.
